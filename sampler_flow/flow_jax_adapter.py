...
_FLOW_PROGRESS_BAR = {"bar": None}


def _flow_progress_callback(epoch, total_epochs, loss):
    """
    Host-side tqdm update called from jitted JAX code via jax.debug.callback.
    """
    epoch = int(np.asarray(epoch))
    total_epochs = int(np.asarray(total_epochs))
    loss = float(np.asarray(loss))

    bar = _FLOW_PROGRESS_BAR["bar"]

    if epoch == 0 or bar is None or bar.total != total_epochs:
        if bar is not None:
            bar.close()
        print("Training flow...")
        bar = tqdm(total=total_epochs, dynamic_ncols=True, leave=True)
        _FLOW_PROGRESS_BAR["bar"] = bar

    target_n = epoch + 1
    if bar.n < target_n:
        bar.update(target_n - bar.n)

    bar.set_postfix_str(f"train={loss:.3g}, val={loss:.3g}")

    if target_n >= total_epochs:
        bar.close()
        _FLOW_PROGRESS_BAR["bar"] = None
...
def create_flow(
    ...
    """
    Version-robust FlowJAX flow constructor.
    """
...
class FlowJAXAdapter:
    ...
    def tree_flatten(self):
        # Now include show_progress in aux data
        return (self.params, self.opt_state), (self.static, self.dim, self.show_progress)

    
    @classmethod
    def tree_unflatten(cls, aux, children):
        static, dim, show_progress = aux
    ...
    def fit(
        ...
        *,
        ...
        def _do_fit(op):
            ...
            epoch_ids = jnp.arange(max_epochs, dtype=jnp.int32)
            total_epochs_arr = jnp.asarray(max_epochs, dtype=jnp.int32)

            def _epoch_body(flow_state_inner, xs):
                epoch_idx, key_epoch = xs
                flow_state_inner, epoch_loss = _train_one_epoch(flow_state_inner, key_epoch)

                if show_progress:
                    jax.debug.callback(
                        _flow_progress_callback,
                        epoch_idx,
                        total_epochs_arr,
                        epoch_loss,
                        ordered=True,
...
def init_flow_state(
    ...
    *,
    ...
    nn_depth: int = 3,
    learning_rate: float = 5e-3,
    show_progress: bool = False,  # Add this parameter
) -> FlowJAXAdapter:
    """
    ...
    return FlowJAXAdapter(
        ...
        opt_state=opt_state,
        dim=n_dim,
        # Note: show_progress is stored in the adapter for use during fit
        show_progress=show_progress,  # You'll need to add this to FlowJAXAdapter class
    )