...
class FlowDiagnosticsJAX:
    ...
    def tree_unflatten(cls, aux_data, children):
        trained, losses, flow_samples, weights, keep_mask, beta = children
        return cls(
            trained=trained,
            losses=losses,
...
class RunOutputJAX:
    ...
    logz: Array
    logz_err: Array
    flow_diagnostics: FlowDiagnosticsJAX


    def tree_flatten(self):
        return (self.state, self.logz, self.logz_err, self.flow_diagnostics), None


    @classmethod
    def tree_unflatten(cls, aux_data, children):
        state, logz, logz_err, flow_diagnostics = children
        return cls(
            state=state,
            logz=logz,
            logz_err=logz_err,
    ...
    *,
    ...
    def _run(
        ...
        *,
        ...
        flow_diag0 = FlowDiagnosticsJAX(
            trained=jnp.zeros((n_max_steps,), dtype=jnp.bool_),
            losses=jnp.full((n_max_steps, train_cfg.max_epochs), jnp.nan, dtype=dtype),
            flow_samples=jnp.full((n_max_steps, keep_max, 2), jnp.nan, dtype=dtype),
            weights=jnp.zeros((n_max_steps, keep_max), dtype=dtype),
            keep_mask=jnp.zeros((n_max_steps, keep_max), dtype=jnp.bool_),
            beta=jnp.full((n_max_steps,), jnp.nan, dtype=dtype),
        )



        def cond_fn(carry):
            key_c, state_c, cur_c, geom_c, n_eff_c2, it, flow_c, flow_diag_c = carry
            del flow_c, flow_diag_c
        ...
        def body_fn(carry):
            ...
            def _do_train(op):
                ...
                return key_b, flow_new, losses, jnp.asarray(True)

            def _skip_train(op):
                key_b, flow_b, _u_b, _w_b = op
                losses = jnp.full((train_cfg.max_epochs,), jnp.nan, dtype=dtype)
                return key_b, flow_b, losses, jnp.asarray(False)

            key_c, flow_c, flow_losses_iter, did_train = lax.cond(
                should_train,
                _do_train,
                _skip_train,
            ...
            key_c, key_flowplot = jax.random.split(key_c)
            flow_draws = flow_c.sample(key_flowplot, keep_max)
            theta_plot = jnp.zeros((keep_max, 2), dtype=dtype)
            theta_plot = theta_plot.at[:, 0].set(flow_draws[:, 0])
            if cfg.n_dim > 1:
                theta_plot = theta_plot.at[:, 1].set(flow_draws[:, 1])

            idx0 = jnp.asarray(0, dtype=it.dtype)


            flow_diag_c = FlowDiagnosticsJAX(
                trained=lax.dynamic_update_slice(
                    flow_diag_c.trained,
                    did_train[None],
                    (it,),
                ),
                losses=lax.dynamic_update_slice(
                    flow_diag_c.losses,
                    flow_losses_iter[None, :],
                    (it, idx0),
                ),
                flow_samples=lax.dynamic_update_slice(
                    flow_diag_c.flow_samples,
                    theta_plot[None, :, :],
                    (it, idx0, idx0),
                ),
                weights=lax.dynamic_update_slice(
                    flow_diag_c.weights,
                    cur_rw["weights"][None, :],
                    (it, idx0),
                ),
                keep_mask=lax.dynamic_update_slice(
                    flow_diag_c.keep_mask,
                    cur_rw["keep_mask"][None, :],
                    (it, idx0),
                ),
        ...
        key, state, cur, geom, n_eff_c, itf, flow_final, flow_diag_final = lax.while_loop(
            cond_fn,
            body_fn,
            (key, state, current_particles0, geom, n_eff_c, iter0, flow_obj0, flow_diag0),
        )
        ...
        return RunOutputJAX(state=state, logz=logz_final, logz_err=logz_err, flow_diagnostics=flow_diag_final,)
